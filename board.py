...
def clean(board, priority):
    ...
    def kids(position):
        potentials = [position + 1, position - 1, position + 9, position - 9]
        potentials = [i for i in potentials if i >= 0 and i < 81 and abs(position % 9 - i % 9) < 2]
        return potentials
...
class Board:
    ...
    def from_letters(self, move):
        ...
        column = "ABCDEFGHI".index("".join([i.upper() for i in move if i.isalpha()]))
        return row-1, column
    ...
    def move(self, move, who):
        ...
        if move == 81 or str(move).lower() == "pass":
            self.moves += 1
    ...
    def to_array(self):
        ...
        x = (-1)**(self.moves)
        moves = np.array([[[x]]*9]*9)
        return np.concatenate([moves, pos0, neg0, pos1, neg1, pos2, neg2], axis=2)
    
    def score(self):
        return 1 if np.sum(self.board[-1]) > 5.5 * (-1)**(self.moves) else -1
